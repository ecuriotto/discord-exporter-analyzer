...
def get_wordcloud_img(df):
    ...
    stopwords_path = os.path.join(os.path.dirname(__file__), 'analysis/resources', 'italian_stopwords.txt')
...
def get_spammer_chart(df, top_n=10):
    ...
    fig = go.Figure(go.Bar(
        x=spammer_counts.values,
        y=spammer_counts.index,
        orientation='h',
        marker=dict(
            color=spammer_counts.values,
            colorscale='YlOrRd', 
            showscale=False
        ),
        text=spammer_counts.values,
        textposition='auto'
    ))
